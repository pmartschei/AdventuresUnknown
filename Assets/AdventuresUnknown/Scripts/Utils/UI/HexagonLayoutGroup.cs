...
{
    [AddComponentMenu("Layout/Hexagon Layout Group", 153)]
    public class HexagonLayoutGroup : LayoutGroup
    {
        ...
        public override void CalculateLayoutInputVertical()
        {
            ...
            else if (m_Constraint == Constraint.FixedRowCount)
            {
                ...
                float height = this.rectTransform.rect.height / minRows;
                height *= 1.125f;
                cellSize = new Vector2(height,height);
                spacing = new Vector2(0, -height * 0.125f);
            }
            else
            {
        ...
        private void SetCellsAlongAxis(int axis)
        {
            ...
            for (int i = 0; i < rectChildren.Count; i++)
            {
                ...
                if (startAxis == Axis.Horizontal)
                {
                    //modified for hexagon
                    int modifiedI = i;
                    positionY = modifiedI / (cellsPerMainAxis * 2 - 1);
                    modifiedI -= positionY * (cellsPerMainAxis * 2 - 1);
                    positionY *= 2;
                    positionX = modifiedI % cellsPerMainAxis;
                    positionY += modifiedI / cellsPerMainAxis;
                    //end modified
                }
                else
                {
                ...
                if (positionY % 2 == 1)
                {
                    SetChildAlongAxis(rectChildren[i], 0, startOffset.x + (cellSize[0] + spacing[0]) * (positionX+0.5f), cellSize[0]);
                }
                else
                {
